    ...
    const handleTransfer = async (dados: any) => {
        ...
        if (window.confirm('Deseja transferir o imóvel? Esta ação não pode ser desfeita.')) {
            ...
            if (responseDelete instanceof Error) {
                ...
                const responseCreate = await controllerImoveis.createURI({
                    ...imovel,
                    conta: novaConta
                });
                if (responseCreate instanceof Error) {
                    toast.error('Erro ao atualizar o token.');
                } else {
                    const responseTranfer = await contract.methods.transferirToken(conta, novaConta, imovel.id, responseCreate.uri).send({
                        from: conta
                    });
                    setIsLoading(false);
